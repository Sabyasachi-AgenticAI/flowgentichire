        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-fade-up">
          ...
          {candidates.length > 0 && (
            <div className="px-5 py-4 border-t border-slate-100 bg-slate-50/40 space-y-3">
              ...
              <div className="flex items-center justify-between px-1">
                <div>
                  <p className="text-xs font-semibold text-slate-700">
                    {callMode === "auto" ? "Automatic mode" : "Manual mode"}
                  </p>
                  <p className="text-xs text-slate-400 mt-0.5">
                    {callMode === "auto"
                      ? "AI calls candidates one by one automatically"
                      : "You approve each call before it goes out"}
                  </p>
                </div>
                <button
                  onClick={handleToggleMode}
                  disabled={modeLoading}
                  className="flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 disabled:opacity-50 transition-colors shrink-0"
                >
                  <div className={`relative w-9 h-5 rounded-full transition-colors duration-200 ${callMode === "auto" ? "bg-indigo-500" : "bg-slate-300"}`}>
                    <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform duration-200 ${callMode === "auto" ? "translate-x-4" : "translate-x-0.5"}`} />
                  </div>
                  <span className={`text-xs font-semibold ${callMode === "auto" ? "text-indigo-600" : "text-slate-500"}`}>
              ...
              {selected.size === 0 && (
                <p className="text-center text-xs text-slate-400">Select at least one candidate to call</p>
              )}
            </div>
          )}
        </div>
