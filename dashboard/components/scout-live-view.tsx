...
function CallStatusBadge({ rc }: { rc: MatchedCandidate }) {
  ...
  if (summary && !assessment)
    return (
      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border border-amber-300 text-amber-700 bg-amber-50">
...
export default function ScoutLiveView({
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      await loadData();
      // clear the "just launched" flag after animation completes
            ...
            {total > 0 && (
              <div className="px-5 py-4 border-t border-slate-100 animate-slide-down" style={{ animationDelay: "200ms", opacity: 0, animationFillMode: "forwards" }}>
                ...
                <div className="flex items-center gap-5 mt-2.5 text-xs text-slate-500">
                  ...
                  <span><strong className="text-slate-700">{total - called}</strong> remaining</span>
